...
class ManageTrigger:
    ...
    def __init__(self, prometheus_adapter_id, user):
        """
        Prometheus監視マスタID=prometheus_adapter_idとなるPrometheus障害取得履歴管理を取得する
        """
    ...
    def main(self, triggerid_lastchange_list):
        """
        [概要]
        障害取得管理を行う。Prometheusから取得したtriggerid,lastchangeと、
        ...
        Prometheus apiで得られるtriggeridとlastchangeのsetのリスト
        [戻り値]
    ...
    def create(self, trigger_id, lastchange, user):
        ...
        レコード作成
        [戻り値]
        PrometheusTriggerHistory: 作成したモデルを返す。　例外の場合はNoneを返す
        """
        logger.logic_log('LOSI00001', 'trigger_id: %s, lastchange: %s, user_name: %s' % (trigger_id, lastchange, user))

        prometheus_trigger_his = PrometheusTriggerHistory(
        ...
        prometheus_trigger_his.save(force_insert=True)

        logger.logic_log('LOSI00002', 'prometheus_trigger_history_id: %s' % (prometheus_trigger_his))
        return prometheus_trigger_his
    ...
    def update(self, trigger_his, lastchange, user):
        ...
        logger.logic_log('LOSI00002', 'prometheus_trigger_history_id: %s' % (trigger_his))
    ...
    def delete_resolved_records(self, trigger_history_list, active_trigger_id_list):
        ...
        解決済みになった障害は削除する
        [引数]
        trigger_history_list: テーブルに登録されているPrometheusTriggerHistoryのクラスのリスト
        active_trigger_id_list: Prometheusから取得したトリガーIDのリスト
        """
        for t in trigger_history_list:
            if not t.trigger_id in active_trigger_id_list:
                logger.logic_log('LOSI30003', self.prometheus_adapter_id, t.trigger_id)
                t.delete()
