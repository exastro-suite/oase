...
from web_app.models.Grafana_monitoring_models import GrafanaTriggerHistory 
from libs.backyardlibs.monitoring_adapter.Grafana.Grafana_api import GrafanaApi


class ManageTrigger:

    def __init__(self, grafana_adapter_id, user):
        """
        Grafana監視マスタID=grafana_adapter_idとなるGrafana障害取得履歴管理を取得する
        """
    ...
    def main(self, triggerid_lastchange_list):
        """
        [概要]
        障害取得管理を行う。Grafanaから取得したtriggerid,lastchangeと、
        ...
        Grafana apiで得られるtriggeridとlastchangeのsetのリスト
        [戻り値]
        ...
        trigger_history_list = GrafanaTriggerHistory.objects.select_for_update().filter(grafana_adapter_id=self.grafana_adapter_id)
    ...
    def create(self, trigger_id, lastchange, user):
        ...
        grafana_trigger_his.save(force_insert=True)

        logger.logic_log('LOSI00002', 'grafana_trigger_history_id: %s' % (grafana_trigger_his))
        return grafana_trigger_his
    ...
    def update(self, trigger_his, lastchange, user):
        ...
        logger.logic_log('LOSI00002', 'grafana_trigger_history_id: %s' % (trigger_his))
    ...
    def delete_resolved_records(self, trigger_history_list, active_trigger_id_list):
        ...
        解決済みになった障害は削除する
        [引数]
        trigger_history_list: テーブルに登録されているGrafanaTriggerHistoryのクラスのリスト
        active_trigger_id_list: Grafanaから取得したトリガーIDのリスト
        """
