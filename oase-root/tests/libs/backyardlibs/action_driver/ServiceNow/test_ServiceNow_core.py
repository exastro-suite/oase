...
def test_send_put_request_ok(monkeypatch):
    """
    PUTリクエスト送信のテスト
    正常系
    """
    ...
    def put_dummy_true(*args, **kwargs):
        """正常系用 request.postの戻り値"""
        return DummyTrue()
    ...
    ary_data = {}
    ary_data['state'] = '7'
    ary_data['close_notes'] = 'close'
    ary_data['close_code'] = 'Closed/Resolved by Caller'
    str_para_json_encoded = json.dumps(ary_data, default=str)
    ...
    monkeypatch.setattr(requests, 'put', put_dummy_true)
    result = core.send_put_request(url, user, passwd, str_para_json_encoded.encode('utf-8'), headers, proxies)
    assert(result)


def test_send_put_request_ng(monkeypatch):
    """
    PUTリクエスト送信のテスト
    異常系
    """
    ...
    def put_dummy_false(*args, **kwargs):
        """異常系用 request.putの戻り値"""
        return DummyFalse()
    ...
    monkeypatch.setattr(requests, 'put', put_dummy_false)
    result = core.send_put_request(url, user, passwd, str_para_json_encoded.encode('utf-8'), headers, proxies)
    assert result != 201


@pytest.mark.django_db
def test_modify_incident_ok(monkeypatch):
    """
    インシデント管理チケットクローズのテスト
    正常系
    """
    class DummyData:
        servicenow_driver_id = 1
    ...
    monkeypatch.setattr(ServiceNow1Core, 'send_put_request', method_dummy_true)
    result = core.modify_incident(drv, sys_id, ary_data)
    assert(result)


@pytest.mark.django_db
def test_modify_incident_ng(monkeypatch):
    """
    インシデント管理チケットクローズのテスト
    異常系
    """
    ...
    monkeypatch.setattr(ServiceNow1Core, 'send_put_request', method_dummy_false)
    result = core.modify_incident(drv, sys_id, ary_data)
    assert result == False
