...
class TestBlackList:
    ...
    def test_index(self, admin):
        ...
        登録0件時の表示テスト
        """
        response = admin.get(reverse('web_app:user:black_list'))
        content = response.content.decode('utf-8')

        assert response.status_code == 200
        assert get_message('MOSJA34000', 'JA', showMsgId=False) in content
    ...
    def test_index2(self, admin):
        ...
        assert response.status_code == 200
        assert get_message('MOSJA34016', 'JA', showMsgId=False) in content
